    ...
    <div id="uniedd-calendar" style={{ background:'rgba(255,255,255,0.04)', border:'0.5px solid rgba(255,255,255,0.07)', borderRadius:'14px', padding:'1rem', marginTop:'14px' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(7,1fr)', gap:'2px' }}>
        {cells.map((day, i) => {
          ...
          return (
            <div key={day} onClick={() => setSelected(isSelected ? null : day)} style={{ minHeight:'44px', borderRadius:'8px', padding:'4px', background: isSelected ? 'rgba(30,144,255,0.2)' : isToday ? 'rgba(30,144,255,0.08)' : 'rgba(255,255,255,0.03)', border: isSelected ? '1px solid #1e90ff' : isToday ? '1px solid rgba(30,144,255,0.3)' : '1px solid rgba(255,255,255,0.05)', cursor:'pointer', transition:'all 0.15s' }}>
            ...
            <form onSubmit={addEvent} style={{ background:'rgba(255,255,255,0.05)', borderRadius:'10px', padding:'12px', marginBottom:'10px' }}>
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'8px', marginBottom:'8px' }}>
                <div>
                  ...
                  <input value={form.title} onChange={e => setForm(f => ({...f, title:e.target.value}))} required placeholder="Event title" style={{ width:'100%', background:'rgba(255,255,255,0.05)', border:'0.5px solid rgba(255,255,255,0.1)', borderRadius:'8px', padding:'8px 10px', fontSize:'13px', color:'#fff', outline:'none' }}/>
                </div>
                <div>
                  ...
                  <input type="time" value={form.time} onChange={e => setForm(f => ({...f, time:e.target.value}))} style={{ width:'100%', background:'rgba(255,255,255,0.05)', border:'0.5px solid rgba(255,255,255,0.1)', borderRadius:'8px', padding:'8px 10px', fontSize:'13px', color:'#fff', outline:'none' }}/>
                </div>
              </div>
              <div style={{ marginBottom:'8px' }}>
                ...
                <select value={form.type} onChange={e => setForm(f => ({...f, type:e.target.value}))} style={{ width:'100%', background:'rgba(255,255,255,0.07)', border:'0.5px solid rgba(255,255,255,0.1)', borderRadius:'8px', padding:'8px 10px', fontSize:'13px', color:'#fff', outline:'none' }}>
              ...
              <div style={{ display:'flex', gap:'8px' }}>
                <button type="submit" disabled={saving} style={{ flex:1, padding:'8px', background:'#1e90ff', color:'#fff', border:'none', borderRadius:'8px', fontSize:'13px', fontWeight:700, cursor:'pointer', opacity:saving?0.6:1 }}>{saving ? 'Saving...' : 'Add Event'}</button>
                <button type="button" onClick={() => setShowAdd(false)} style={{ padding:'8px 16px', background:'rgba(255,255,255,0.07)', color:'rgba(255,255,255,0.5)', border:'none', borderRadius:'8px', fontSize:'13px', cursor:'pointer' }}>Cancel</button>
              </div>
            </form>
          )}
            ...
            : selectedEvents.map((ev, i) => (
              <div key={i} style={{ display:'flex', alignItems:'center', gap:'10px', padding:'8px 10px', background:'rgba(255,255,255,0.04)', borderRadius:'8px', marginBottom:'6px', border:`0.5px solid ${TYPE_COLOR[ev.event_type] || '#888'}33` }}>
                ...
                <div style={{ flex:1 }}>
                  ...
                  <div style={{ fontSize:'11px', color:'rgba(255,255,255,0.3)', marginTop:'1px' }}>{ev.time || ev.start_time || ''} {ev.event_type && `· ${ev.event_type}`} {ev.batch && `· ${ev.batch}`} {ev.teacher_name && `· 👨‍🏫 ${ev.teacher_name}`}</div>
                </div>
                {ev.meet_link && (
                  <a href={ev.meet_link} target="_blank" rel="noreferrer" style={{ fontSize:'10px', fontWeight:700, padding:'3px 10px', borderRadius:'8px', background:'rgba(30,144,255,0.15)', color:'#5aabff', border:'0.5px solid rgba(30,144,255,0.25)', whiteSpace:'nowrap' }}>Join Zoom</a>
                )}
              </div>
            ))
          }
