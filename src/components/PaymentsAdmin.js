...
// USD only — PayPal works natively with USD
const CURRENCIES = [
...
export default function PaymentsAdmin({ profile }) {
  ...
  // Call Vercel serverless → PayPal API
  async function createPayPalLink(amt, curr, desc, studentName, studentEmail, invoiceNo) {
    const res = await fetch('/api/create-paypal-link', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ amount: amt, currency: curr, description: desc, studentName, studentEmail, invoiceNo }),
    })
  ...
  async function generate(e) {
    ...
    try {
      const result = await createPayPalLink(amtNum, currency, desc, student?.full_name, student?.email, invoiceNo)
      ...
      setGenerated({ link:result.paymentLink, invoiceNo, studentName:student?.full_name, amount:amtNum, currency, course:courseName.trim() })
      setOk(`✓ PayPal link generated — ${invoiceNo}`)
      setSelStudent(''); setCourseName(''); setAmount(''); setCurrency('USD'); setDueDate(''); setNotes('')
      load()
  ...
  return (
    <div id="uniedd-payments" style={{ marginTop:'14px' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'10px', marginBottom:'14px' }}>
        {[
          { label:'Collected',   value:`$${totalPaid.toFixed(2)}`,  color:'#10b981', bg:'rgba(16,185,129,0.1)',  bd:'rgba(16,185,129,0.2)'  },
          { label:'Pending',     value:`$${totalDue.toFixed(2)}`,    color:'#f4a335', bg:'rgba(232,124,30,0.1)', bd:'rgba(232,124,30,0.2)'  },
        ...
        <div style={{ background:'rgba(255,255,255,0.04)', border:'0.5px solid rgba(255,255,255,0.07)', borderRadius:'14px', padding:'1.2rem' }}>
          <div style={{ display:'flex', alignItems:'center', gap:'12px', padding:'12px 14px', background:'rgba(24,119,242,0.08)', border:'0.5px solid rgba(24,119,242,0.2)', borderRadius:'10px', marginBottom:'18px' }}>
            ...
            <div style={{ flex:1 }}>
              ...
              <div style={{ fontSize:'11px', color:'rgba(255,255,255,0.4)', marginTop:'2px' }}>Fill the form → Generate → Real PayPal payment link instantly.</div>
            </div>
            <span style={{ fontSize:'10px', fontWeight:700, padding:'4px 10px', borderRadius:'20px', background:'rgba(16,185,129,0.15)', color:'#10b981', border:'0.5px solid rgba(16,185,129,0.3)', flexShrink:0 }}>✓ Live API</span>
          </div>
          ...
          <form onSubmit={generate}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px' }}>
              <div>
                ...
                <div style={{ position:'relative' }}>
                  <span style={{ position:'absolute', left:'13px', top:'50%', transform:'translateY(-50%)', color:'rgba(255,255,255,0.4)', fontSize:'14px' }}>{getSymbol(currency)}</span>
                  <input style={{ ...inp, paddingLeft:'28px' }} type="number" placeholder="60" min="1" step="0.01" value={amount} onChange={e=>setAmount(e.target.value)} required />
                </div>
              </div>
              <div>
            ...
            <button type="submit" disabled={busy} style={{ width:'100%', padding:'14px', background:busy?'rgba(24,119,242,0.4)':'linear-gradient(135deg,#1877f2,#0ea5e9)', color:'#fff', fontSize:'15px', fontWeight:800, border:'none', borderRadius:'10px', cursor:busy?'not-allowed':'pointer', marginTop:'14px', fontFamily:'inherit', display:'flex', alignItems:'center', justifyContent:'center', gap:'8px' }}>
              {busy ? <><span style={{ display:'inline-block', width:'16px', height:'16px', border:'2px solid rgba(255,255,255,0.3)', borderTopColor:'#fff', borderRadius:'50%', animation:'spin .7s linear infinite' }}/> Generating...</> : <>🅿️ Generate PayPal Payment Link</>}
            </button>
          </form>
          ...
          {generated && (
            <div style={{ marginTop:'18px', padding:'16px', background:'rgba(16,185,129,0.07)', border:'1px solid rgba(16,185,129,0.25)', borderRadius:'14px' }}>
              ...
              <div style={{ fontSize:'12px', color:'rgba(255,255,255,0.5)', marginBottom:'10px' }}>
                ...
                {' · '}
                <span style={{ color:'#10b981', fontWeight:700 }}>{getSymbol(generated.currency)}{generated.amount} {generated.currency}</span>
                {' · '}{generated.course}
              </div>
        ...
        <div style={{ background:'rgba(255,255,255,0.04)', border:'0.5px solid rgba(255,255,255,0.07)', borderRadius:'14px', padding:'1.1rem 1.2rem' }}>
          ...
          {loading ? (
            ...
            <div style={{ display:'grid', gap:'8px' }}>
              {filtered.map(p=>{
                const sym = getSymbol(p.currency||'USD')
                const displayAmt = `${sym}${(p.amount||0).toFixed(2)} ${p.currency||'USD'}`
                return (
