...
export default function AdminDash({ profile }) {
  ...
  return (
    <Layout profile={profile} pageTitle="Dashboard">
      ...
      {/* ── ADMIN PANEL LAYOUT: LEFT NAV + CONTENT ── */}
      <div style={{ display:'grid', gridTemplateColumns:'200px 1fr', gap:'16px', alignItems:'start' }}>

        {/* ── LEFT SIDEBAR NAV ── */}
        <div style={{ background:'rgba(255,255,255,0.03)', border:'0.5px solid rgba(255,255,255,0.07)', borderRadius:'14px', padding:'10px', position:'sticky', top:'20px' }}>
          {[
            { group:'OVERVIEW',   items:[
              { id:'overview',    icon:'📊', label:'Dashboard'      },
            ]},
            { group:'LEADS',      items:[
              { id:'leads',       icon:'📋', label:'All Leads',    badge: stats.newLeads > 0 ? stats.newLeads : null },
              { id:'adleads',     icon:'📣', label:'From Ads',     badge: adLeads.filter(l=>l.status==='New').length || null },
            ]},
            { group:'STUDENTS',   items:[
              { id:'users',       icon:'👥', label:'Users & Roles'  },
              { id:'enrol',       icon:'🎓', label:'Enrol Student'  },
              { id:'payments',    icon:'💳', label:'Payments'       },
              { id:'feereminder', icon:'🔔', label:'Fee Reminders'  },
            ]},
            { group:'CLASSES',    items:[
              { id:'classes',     icon:'📅', label:'All Classes'    },
              { id:'schedule',    icon:'➕', label:'Schedule Class' },
              { id:'reschedule',  icon:'↺',  label:'Reschedule'     },
              { id:'batchreset',  icon:'🔄', label:'Manage Student' },
              { id:'bulkcancel',  icon:'🚫', label:'Bulk Cancel'    },
            ]},
            { group:'CONTENT',    items:[
            ...
            <div key={group.group} style={{ marginBottom:'6px' }}>
              <div style={{ fontSize:'9px', fontWeight:800, color:'rgba(255,255,255,0.2)', letterSpacing:'0.12em', padding:'8px 8px 4px' }}>{group.group}</div>
              {group.items.map(item => (
                <button key={item.id} onClick={() => setActiveSection(item.id)} style={{
                  width:'100%', display:'flex', alignItems:'center', gap:'8px',
                  padding:'8px 10px', borderRadius:'9px', border:'none', cursor:'pointer',
                  fontFamily:'inherit', fontSize:'12px', fontWeight:activeSection===item.id?700:500,
                  background: activeSection===item.id ? 'rgba(30,144,255,0.18)' : 'transparent',
                  color:      activeSection===item.id ? '#fff' : 'rgba(255,255,255,0.45)',
                  textAlign:'left', marginBottom:'1px', position:'relative',
                  borderLeft: activeSection===item.id ? '3px solid #1e90ff' : '3px solid transparent',
                }}>
                  <span style={{ fontSize:'13px', width:'18px', flexShrink:0 }}>{item.icon}</span>
                  <span style={{ flex:1 }}>{item.label}</span>
                  {item.badge > 0 && (
                    <span style={{ fontSize:'9px', fontWeight:800, padding:'1px 6px', borderRadius:'8px', background:'#f4a335', color:'#000', flexShrink:0 }}>{item.badge}</span>
                  )}
                </button>
              ))}
            </div>
